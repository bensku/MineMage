            ...
            for (int l = -2; l <= 2; ++l)
            {
                for (int i1 = -2; i1 <= 2; ++i1)
                {
                    for (int j1 = -1; j1 < 3; ++j1)
                    {
                        ...
                        int i2 = k + i1 * b1 - l * b0;
                        boolean flag = j1 < 0;
                        this.worldServerInstance.setBlock(k1, l1, i2, flag ? Block.stone.blockID : 0);
                    }
                }
            }
...
*/
    public boolean placeInExistingPortal(Entity par1Entity, double par2, double par4, double par6, float par8)
    {
        ...
        {
            PortalPositionSky portalposition = (PortalPositionSky)this.field_85191_c.getValueByKey(j1);
            d3 = 0.0D;
        ...
        else
        {
            for (k1 = l - short1; k1 <= l + short1; ++k1)
            {
                ...
                for (int l1 = i1 - short1; l1 <= i1 + short1; ++l1)
                {
                    ...
                    {
                        if (this.worldServerInstance.getBlockId(k1, i2, l1) == MainMod.BlockSkyPortal.blockID)
                        {
                            while (this.worldServerInstance.getBlockId(k1, i2 - 1, l1) == MainMod.BlockSkyPortal.blockID)
                            {
                                --i2;
                            }
        ...
        if (d3 >= 0.0D)
        {
            if (flag)
            {
                this.field_85191_c.add(j1, new PortalPositionSky(this, i, j, k, this.worldServerInstance.getTotalWorldTime()));
            ...
            if (this.worldServerInstance.getBlockId(i - 1, j, k) == MainMod.BlockSkyPortal.blockID)
            {
                j2 = 2;
            }
            ...
            if (this.worldServerInstance.getBlockId(i, j, k - 1) == MainMod.BlockSkyPortal.blockID)
            {
                j2 = 3;
            }
        ...
        for (k2 = 0; k2 < 4; ++k2)
        {
            for (i3 = 0; i3 < 4; ++i3)
            {
                for (l2 = -1; l2 < 4; ++l2)
                {
                    ...
                    this.worldServerInstance.setBlock(k3, j3, i4, flag ? Block.stone.blockID : MainMod.BlockSkyPortal.blockID, 0, 2);
                }
            }
    ...
    public void func_85189_a(long par1)
    {
        if (par1 % 100L == 0L)
        {
            ...
            while (iterator.hasNext())
            {
                ...
                PortalPositionSky portalposition = (PortalPositionSky)this.field_85191_c.getValueByKey(olong.longValue());
